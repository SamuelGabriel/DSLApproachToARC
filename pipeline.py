...
from policy import StateActionProgram, PLPPolicy, ARCPLPPolicy
...
def get_program_set(base_class_name, num_programs):
    ...
    grammar = create_grammar(object_types, arc_grammar=arc_tasks.prefix.format('') in base_class_name)
...
def extract_examples_from_demonstration_item(demonstration_item):
    ...
    if len(action) == 3:
        for color in range(len(arc_tasks.color_map)):
            if action[2] != color:
                wrong_action = (action[0], action[1], color)
                negative_examples.append((state, wrong_action))
    else:
...
def run_all_programs_on_single_demonstration(base_class_name, num_programs, demo_number, program_interval=1000):
    ...
    #print('Example out',[(i,programs[10](*positive_examples[i])) for i in range(30)])
    #print('For Example', positive_examples[:30])
...
def run_all_programs_on_demonstrations(base_class_name, num_programs, demo_numbers):
    ...
    #print('Copy Program',X.toarray()[:,10])
...
def learn_single_batch_decision_trees(y, num_dts, X_i):
    ...
    for seed in range(num_dts):
        ...
        #print('y elements',np.unique(y), y.shape)
        #print("x_i elements", X_i.nnz, X_i.shape)
...
def learn_plps(X, y, programs, program_prior_log_probs, num_dts=5, program_generation_step_size=10):
    ...
    for i in range(0, num_programs, program_generation_step_size):
        print("\rLearning plps with {} programs".format(i), end='')
        for clf in learn_single_batch_decision_trees(y, num_dts, X[:, :i+1]):
            #print(clf.n_features_, clf.n_outputs_)
...
def compute_likelihood_single_plp(demonstrations, plp):
    ...
    for obs, action in demonstrations:
        ...
        if len(action) == 2:
            ...
            for c in range(len(arc_tasks.color_map)):
                if c != action[2]:
                    if plp(obs, (action[0],action[1],c)):
                        size+=1
...
def train(base_class_name, demo_numbers, program_generation_step_size, num_programs, num_dts, max_num_particles):
    ...
    plppolicy_class = ARCPLPPolicy if arc_tasks.prefix.format('') in base_class_name else PLPPolicy
    if len(top_particle_log_probs) > 0:
        ...
        policy = plppolicy_class(top_particles, top_particle_probs)
    else:
        print("no nontrivial particles found")
        policy = plppolicy_class([StateActionProgram("False")], [1.0])
...
if __name__  == "__main__":
    ...
    for task in arc_tasks.demonstrations_dict:
        #task = arc_tasks.prefix.format(2)
        examples = (0,)
        policy = train(task, examples, 10, 100, 5, 25)
        test_results = test(policy, task, examples, record_videos=False)
        print("Test results:", test_results)
        overall_acc.append(test_results)
    print('Micro acc', sum(overall_acc)/len(overall_acc))
    print("Macro acc", sum(acc == 1. for acc in overall_acc)/len(overall_acc))
