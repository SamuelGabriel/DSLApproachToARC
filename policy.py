...
class ARCPLPPolicy(PLPPolicy):
    ...
    def hash_obs(self, obs, pos):
        return (tuple(tuple(l) for l in obs),tuple(pos))

    def get_action_probs(self, obs, pos):
        hashed_obs = self.hash_obs(obs, pos)
        if hashed_obs in self._action_prob_cache:
            return self._action_prob_cache[hashed_obs]

        action_probs = np.zeros([len(arc_tasks.color_map)], dtype=np.float32)

        for plp, prob in zip(self.plps, self.probs):
            for c in self.get_plp_suggestions(plp, obs, pos):
                action_probs[c] += prob
